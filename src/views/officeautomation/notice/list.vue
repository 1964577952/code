<template>
    <div class="app-container aside-container" >
        ...
        <div class="filter-container" style="margin-top:20px;">
            <el-input placeholder="请输入通知公告标题进行搜索" v-model="query.keyword" class="input-with-select" style="width:50%;">
        ...
        <el-table style="margin-top:10px;" :data="list" v-loading.body="listLoading" element-loading-text="Loading"  @selection-change="handleSelectionChange" fit highlight-current-row>
            ...
            <el-table-column align="center" label='通知公告标题'>
                <template slot-scope="scope">
                    {{scope.row.title}}
                </template>
            </el-table-column>
            <el-table-column align="center" label='通知公告类型'>
                <template slot-scope="scope">
  ...
  methods: {
    ...
    getPageList() {
      ...
      if(this.activeName=='first'){
            getReceivednotice(this.query)
            .then(response => {
            //绑定分页列表
            this.list =  response.data.noticePage.list;
            this.total = response.data.noticePage.totalRow;
            //关闭load
