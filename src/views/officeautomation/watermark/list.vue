<template>
    <div class="app-container aside-container">
        ...
        <div class="filter-container" style="margin-top:20px;">
            <el-input placeholder="请输入水印名称进行搜索"  v-model="query.keyword" class="input-with-select" style="width:50%;">
        ...
        <el-table style="margin-top:10px;" :data="list" v-loading.body="listLoading" element-loading-text="Loading"  @selection-change="handleSelectionChange" fit highlight-current-row>
            ...
            <el-table-column type="index" tpye="" align="center" label='序号' width="80">
            ...
            <el-table-column align="center" label='水印名称'>
             ...
             <!-- <el-table-column align="center" label='水印预览'>
                <template slot-scope="scope">
                    <img :src="baseUrl+scope.row.uploadurl"  style="width:50px;height:50px;">
...
components: {
  ...
  data() {
    return {
      ...
      listLoading: null,
      multipleSelection:[],
      baseUrl:'http://211.159.176.177:88/'
    };
  },
  created() {
    this.getPageList();
  ...
  methods: {
    ...
    getPageList() {
      ...
      getwatermarkpage(this.query)
        .then(response => {
          //绑定分页列表
          this.list =  response.data.watermarkPage.list;
          this.total = response.data.watermarkPage.totalRow;
          //关闭load
