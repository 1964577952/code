<template>
    <div class="app-container aside-container">
        ...
        <div class="filter-container" style="margin-top:20px;">
             <el-input placeholder="请输入资料名称进行搜索" v-model="query.keyword" class="input-with-select" style="width:50%;">
        ...
        <el-table style="margin-top:10px;" :data="list" v-loading.body="listLoading" element-loading-text="Loading"  @selection-change="handleSelectionChange" fit highlight-current-row>
            ...
            <el-table-column align="left" label='操作' width="220">
                <template slot-scope="scope">
                    <el-button type="primary"  plain size="mini" @click="handleopen(scope.row)">下载</el-button>
...
components: {
  ...
  methods: {
    ...
    handleopen(row){
            ...
            window.open("http://211.159.176.177:88/"+row.uploadurl);
    },
    handleedit(row){
    ...
    getPageList() {
      ...
      //根据查询条件获取分页数据
      getdatapage(this.query)
        .then(response => {
          //绑定分页列表
          this.list =  response.data.dataPage.list;
          this.total = response.data.dataPage.totalRow;
          //关闭load
          this.listLoading = false;
        })
        .catch(err => {
