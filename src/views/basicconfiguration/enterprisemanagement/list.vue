<template>
    <div class="app-container aside-container">
        ...
        <div class="filter-container" style="margin-top:20px;">
            <el-input placeholder="请输入企业名称/市/企业法人/联系人/地址...进行搜索" v-model="query.keyword" class="input-with-select" style="width:50%;">
        ...
        <el-table style="margin-top:10px;" :data="list" v-loading.body="listLoading" element-loading-text="Loading"  @selection-change="handleSelectionChange" fit highlight-current-row>
            ...
            <el-table-column align="left" label='单位状态' width="100">
              <template slot-scope="scope" >
                    {{scope.row.state == 1 ? '有效':'无效'}}
                </template>
            </el-table-column>
            <el-table-column align="left" label='城市' width="100">
              <template slot-scope="scope" >
                    {{scope.row.city}}
                </template>
            </el-table-column>
            <el-table-column align="left" label='企业法人' width="120">
              <template slot-scope="scope" >
                    {{scope.row.legalPerson}}
                </template>
            </el-table-column>
            <el-table-column align="left" label='联系人' width="120">
              <template slot-scope="scope" >
                    {{scope.row.contacts}}
                </template>
            </el-table-column>
...
components: {
  ...
  methods: {
    ...
    getPageList() {
      ...
      this.query.city= 'like,'+this.query.keyword;
      this.query.legalPerson= 'like,'+this.query.keyword;
      this.query.contacts= 'like,'+this.query.keyword;
      this.query.address= 'like,'+this.query.keyword;
      this.query.m= 'or';
      getenterprisepage(this.query)
        .then(response => {
          //绑定分页列表
          this.list =  response.data.enterprisePage.list;
          this.total = response.data.enterprisePage.totalRow;
          //关闭load
