      ...
      <el-row :gutter="20">
             ...
             <el-col :span="18">
                <div class="tabletitle">
                     <div><svg-icon icon-class="tabcol" />训练类别</div>
                </div>
                ...
                <el-row :gutter="20">
                    <el-col :span="6">
                        <el-form-item label="训练类别">
                            <el-select  v-model="query.pId" placeholder="请选择父类别" >
                              <el-option
...
components: {
  ...
  methods: {
    ...
    getPageList() {
      ...
      //根据查询条件获取分页数据
      getdrilltypepage(this.query)
        .then(response => {
          //绑定分页列表
          this.list =  response.data.drillTypePage.list;
          this.total = response.data.drillTypePage.totalRow;
        ...
        if(currentnode!=null){
           this.$confirm('此操作将删除数据, 是否继续?', '提示', {
                ...
                type: 'warning'
                      ...
                      deletedrilltype({ids:  currentnode.id})
                        .then(response => {
                            Message({
                            message: "删除成功",
                            type: "success"
