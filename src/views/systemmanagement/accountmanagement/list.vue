<template>
    <div class="app-container aside-container">
        ...
        <div class="filter-container" style="margin-top:20px;">
            <el-input placeholder="请输入用户名/昵称...进行搜索" v-model="query.keyword" class="input-with-select" style="width:50%;">
        ...
        <el-table style="margin-top:10px;" :data="list" v-loading.body="listLoading" element-loading-text="Loading"  @selection-change="handleSelectionChange" fit highlight-current-row>
            ...
            <el-table-column align="left" label='操作' width="200">
                <template slot-scope="scope">
                    ...
                    <el-button type="primary"  plain size="mini" @click="handledel(scope.row)">删除</el-button>
                </template>
            </el-table-column>
        </el-table>
...
components: {
  ...
  methods: {
    ...
    getPageList() {
      ...
      //根据查询条件获取分页数据
      getaccountpage(this.query)
        .then(response => {
          //绑定分页列表
          this.list =  response.data.accountPage.list;
          this.total = response.data.accountPage.totalRow;
          //关闭load
          this.listLoading = false;
        })
        .catch(err => {
