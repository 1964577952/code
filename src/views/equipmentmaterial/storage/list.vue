<template>
    <div class="app-container aside-container">
        ...
        <div class="filter-container" style="margin-top:20px;">
            <el-input placeholder="请输入库房编码/库房名称...进行搜索" v-model="query.keyword" class="input-with-select" style="width:50%;">
        ...
        <el-table style="margin-top:10px;" :data="list" v-loading.body="listLoading" element-loading-text="Loading"  @selection-change="handleSelectionChange" fit highlight-current-row>
            ...
            <el-table-column align="center" label='库房编码'>
                <template slot-scope="scope">
                    {{scope.row.code}}
                </template>
            </el-table-column>
            <el-table-column align="center" label='库房名称'>
                <template slot-scope="scope">
                    {{scope.row.name}}
                </template>
            </el-table-column>
            ...
            <el-table-column align="center" label='库管员'>
                <template slot-scope="scope">
                    {{scope.row.keeper}}
                </template>
            </el-table-column>
            <el-table-column align="center" label='库房面积(m²)'>
                <template slot-scope="scope">
                    {{scope.row.area}}
                </template>
            </el-table-column>
            <el-table-column align="center" label='库房类别'>
                <template slot-scope="scope">
                    {{scope.row.type}}
                </template>
            </el-table-column>
            <el-table-column align="center" label='库房等级'>
                <template slot-scope="scope">
                    {{scope.row.level}}
                </template>
            </el-table-column>
            <el-table-column align="center" label='库房电话'>
                <template slot-scope="scope">
                    {{scope.row.phone}}
                </template>
            </el-table-column>
...
components: {
  ...
  methods: {
    ...
    getPageList() {
      ...
      this.query.code= 'like,'+this.query.keyword;
      this.query.m= 'or';
      getstoragepage(this.query)
        .then(response => {
          //绑定分页列表
          this.list =  response.data.storagePage.list;
          this.total = response.data.storagePage.totalRow;
          //关闭load
          this.listLoading = false;
        })
        .catch(err => {
