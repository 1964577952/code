    ...
    <div class="app-container aside-container">
         <el-row :gutter="20">
             ...
             <el-col :span="18">
                <div class="tabletitle">
                     <div><svg-icon icon-class="tabcol" />装备物资类别</div>
                </div>
        ...
        <div class="filter-container" style="margin-top:20px;">
            <el-form label-width="140px">
                <el-row :gutter="20">
                    <el-col :span="6">
                        <el-form-item label="装备材料类型">
                             <el-input v-model="query.type" placeholder="请输入装备装备材料类型"></el-input>
                        </el-form-item>
                    </el-col>
        ...
        <el-table style="margin-top:10px;" :data="list" v-loading.body="listLoading" element-loading-text="Loading"  @selection-change="handleSelectionChange" fit highlight-current-row>
            ...
            <el-table-column align="center" label='装备材料类别'>
                <template slot-scope="scope">
                    {{scope.row.type}}
                </template>
            </el-table-column>
...
components: {
  ...
  methods: {
    ...
    getPageList() {
      ...
      //根据查询条件获取分页数据
      getequipmentTypepage(this.query)
        .then(response => {
          //绑定分页列表
          this.list =  response.data.equipmentTypePage.list;
          this.total = response.data.equipmentTypePage.totalRow;
         ...
         if(currentnode!=null){
           this.$confirm('此操作将删除数据, 是否继续?', '提示', {
                ...
                type: 'warning'
                      ...
                      deleteequipmentType({ids: currentnode.id})
                        .then(response => {
                            Message({
                            message: "删除成功",
                            type: "success"
